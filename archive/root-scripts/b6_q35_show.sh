#!/bin/bash
# 取回 qwen3.5-122b 冒烟结果 + C2 沙箱 (B 站执行)
set -u
python3 - <<'EOF'
import json
with open("/tmp/b6_smoke_qwen35122b.jsonl", encoding="utf-8") as f:
    for line in f:
        r = json.loads(line)
        print(f"########## {r['id']} ({r['elapsed_s']}s, finish={r['finish']}) ##########")
        print("--- CONTENT ---")
        print(r.get("content", r.get("error", ""))[:1500])
...
echo "================ C2 沙箱 ================"
mkdir -p /tmp/b6_c2_q35 && cd /tmp/b6_c2_q35
