#!/bin/bash
# b6_smoke_deepseek.sh — B6 冒烟 deepseek-v4-flash (双机 RPC, 看门狗布防)
# tg ~6.3 t/s 慢路径: 数值 2048 think / 概念 16384 / 代码 6144
set -u
exec 2>&1

# 看门狗: A 站失联 3 次 → 杀测试 (挂死防护, E1 后 cron 已删但规则保留)
cat > /tmp/b6_ds_watchdog.sh <<'WDEOF'
#!/bin/bash
FAIL=0
while true; do
  if ! ping -c 1 -W 2 10.10.10.1 >/dev/null 2>&1; then
    FAIL=$((FAIL+1))
    echo "$(date '+%H:%M:%S') A_UNREACHABLE fail=$FAIL" >> /tmp/b6_ds_watchdog.log
    if [ $FAIL -ge 3 ]; then
      echo "$(date '+%H:%M:%S') WATCHDOG_ABORT killing test" >> /tmp/b6_ds_watchdog.log
      pkill -f 'b6_smoke_deepseek[.]py'
      exit 9
    fi
  else
    FAIL=0
...
cat > /tmp/b6_smoke_deepseek.py <<'PYEOF'
#!/usr/bin/env python3
# b6_smoke_deepseek.py — deepseek-v4-flash 冒烟 5 题 (同套件同参数)
import json, time, urllib.request

API = "http://127.0.0.1:8080/v1/chat/completions"
OUT = "/tmp/b6_smoke_deepseek.jsonl"
...
bash /tmp/b6_ds_watchdog.sh &
WD=$!
echo "watchdog pid=$WD"
python3 /tmp/b6_smoke_deepseek.py
RC=$?
kill $WD 2>/dev/null
echo "=== 测试结束 rc=$RC, watchdog 日志: ==="
cat /tmp/b6_ds_watchdog.log 2>/dev/null || echo "(无失联记录)"
touch /tmp/b6_ds_done
