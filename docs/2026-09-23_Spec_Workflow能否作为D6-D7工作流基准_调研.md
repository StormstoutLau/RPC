...
- **证据等级**: **E1**=本会话实读本仓 · **E2**=子代理实读（两轮全文报告在案，含文件:行号）·
  **E3**=联网一手检索（URL 可核）· **E4**=推断 · **E5**=未找到
...
> 把"基准"拆成 **5 种不同用途**后，结论是：**2 项可以、2 项有条件、1 项必须拒绝**（§2）。
>
> **② 必须拒绝的是"权威源"那一种。** `Spec_Workflow` 的 `ADR-0006` 走的是
> **"指定一处为准、其余跟随"**（方案 B：Spec_Workflow 为权威源 + 指针封堵 + 回流通道），
> 而**它自己的先例就失败了**：`Cpp_Hub` 侧文件被"**冻结于 v1.1**"、靠**指针封堵**防误用、
> **"人工纪律，无自动同步"**（E2，§3.1）。⇒ **这条路在本机群已经试过一次，代价是双份并存。**
>
> **③ ★ 社区给出了正解，而且与 ② 相反 —— SLSA 的范式。**
> SLSA 明确写：**"SLSA is in the eye of the beholder: software consumers ultimately make their own
> SLSA determinations, though in practice they may delegate to some authority."**（E3）
> 且 **"each link in the software supply chain has its own, independent SLSA level — it is not
> transitive"**（**分级不传递**）。
> ⇒ **基准不由生产方宣布，而由消费方持有判定表**。
> ⇒ **D6/D7 应当持有自己的判定表，把 `Spec_Workflow` 当作"词表与形态的来源"，而不是"权威源"。**
>
...
| **K-5 权威源基准** | **❌ 必须拒绝** | ① 它的 ADR-0006 范式**已被自己的先例证伪**（双份并存 + 指针封堵 + 无自动同步）；② 社区范式（SLSA）**明确相反**：消费方自行判定、**分级不传递** | §3.1 · §5.1 |

**⇒ 一句话**：**`Spec_Workflow` 可以作 D6/D7 的"词典"与"形态参照"，但不能作"宪法"。**
...
| 项 | 结果 |
|---|---|
| `Cpp_Hub` 侧 | 文件被标为**"历史副本，冻结于 v1.1"**，靠**指针**防误用 |
| 同步 | **无自动同步**（明确写在决策里） |
| 失效条件 | 三条，含 **"回流频度 <1 次/季度 → 通道名存实亡，重审是否退回方案 A"** |
| 方案 A / C 否决理由 | A："与本仓库 v1.2.1 自包含化、跨项目方法论权威的定位冲突"；C："**放弃治理等于接受未来更大的合并成本**" |

**⇒ 判断**：方案 C（双份独立演化）被以"未来合并成本"否决，**但方案 B 的实际运行结果就是
"一份冻结 + 一份演化 + 靠人工回流"** —— **即用治理的名义实现了方案 C 的实质**。
⇒ **这条路不要再走第二遍**。
...
**⇒ 结论**：**D6/D7 应采用"消费方判定"范式**：
`Spec_Workflow` 提供**词表与形态**，**D6/D7 持有判定表**（谁符合哪一级），**不设"权威源"**。
...
## 6. 建议的落地形态：把 Spec_Workflow **拆成三份可移植制品 + 一份拒绝**

| # | 制品 | 内容 | 落到哪 | 形态 |
|---|---|---|---|---|
| **S-1** | **词表**（✅ 移植） | `A/B/C` + `E1–E5` + 固定状态词表 + `P1/P2/P3` + `P0 审计项` | **`U-2` 字典的"方言之一"** | 作为**映射表的一列**，**不迁移权威源**（§4.1 的教训） |
...
| **S-4** | **权威源身份**（❌ 拒绝） | ADR-0006 式"指定一处为准" | **不采用** | 改用 **SLSA 式消费方判定**（§5.1） |

**⇒ 三条实施纪律**：

1. **用"映射"代替"采纳"**：D6/D7 侧先产出**方言对照表**（`L1/L2/L3` 五义 → 我们的单一轴），
   **映射表本身要被门禁校验**（源词表变了下游要红）——否则重演 `Cpp_Hub`↔`Spec_Workflow` 的漂移。
...
| # | 事项 | 建议 | 依据 |
|---|---|---|---|
| 1 | **是否采用"消费方判定"范式**（D6/D7 自持判定表，**不设权威源**） | **建议采用** —— ADR-0006 范式已被其自身先例证伪；SLSA 给出社区正解 | §3.1 · §5.1 |
| 2 | **`Spec_Workflow` 的定位表述** | 建议写作 **"词表与形态的来源（source of vocabulary & form），非规范源"** | §2 · §6 |
...
| 6 | **本机群若采用"消费方判定"范式，谁持有判定表** | **未设计** —— 候选：`inventory/` 真值表扩展 / `rpc_check.py` 常量层；落入 `U-2` 时专项设计 |
