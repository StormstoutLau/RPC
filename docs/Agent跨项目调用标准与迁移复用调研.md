...
> **⚠ 现状注记 (2026-09-15) —— 本文是 D6 前期调研史档（2026-09-05, v3.5），正文按「两站 (A/B)」写成。
> 站数、跨站链路两项已漂移，阅读时以本块为准；正文不改写（保留溯源）。**
>
> **1. 拓扑：两站 → 三站，且 C 站已实际承担 agent 工作**（2026-09-15 实测，`import` 级验证非文本解析）：
> `~/.claude/skills/` 三站各 12 件（与本文件所载那批同名同构）、`~/.config/opencode/skills/` 各 4 件、
> opencode **1.18.25** + claude **2.1.258** 三站同版；`~/agent-workspaces/` 实有
...
> **§9.9.3 的「L2 前置已解锁」已过期 —— L2 本身于 2026-09-12 实机通过**（详见 [spec/d6-agent-standard/OPEN-ISSUES.md](../spec/d6-agent-standard/OPEN-ISSUES.md) O-11）。
>
...
> |---|---|---|---|---|---|
> | A | ✗ | ✗ | ✗ | ✗ | ✗ |
> | B | 1.12 | 2.5.2 | ✗ | ✗ | **4.3.3**（apt 默认，恰是 G8 明确**不用**的那档） |
> | C | ✗ | ✗ | ✗ | ✗ | ✗ |
>
...
要点：agent CLI 与本地模型后端（nemotron/gpt-oss）全在站上（D5 装备所在）；**免费档（Zen 网关）例外——经 opencode.ai 出站到美国托管，敏感内容不得走免费档（§9.4 sensitivity 纪律）**；主控站只承担任务源、wrapper 封装、产物归档三职责——**主控站不跑 agent、不起服务**（零自加载纪律不破坏）。（v3.4 审计修订：原"模型后端不出站"绝对化表述未随免费默认决策回灌，本句已限定）
...
- ~~JetBrains 调研：双文件团队月均损耗 5.3h~~（**v3.4 审计剔除：无源断言**——原标注"E2 转述"但无 URL 可溯源，专项检索零命中，具体小数点数字为典型幻觉特征；定性结论保留：双文件各自维护存在持续损耗，薄壳导入是止损方向——该定性判断不依赖该无源数据）
...
> **完成情况列（2026-09-16 增补，三站 A/B/C）**：状态均经"统一入口 + 逐条溯源核对"判定（判据：不口头关闭，须有落地资产或已定案），并落了 [OPEN-ISSUES §6](../spec/d6-agent-standard/OPEN-ISSUES.md)。汇总 **8 闭环 / 5 已定案 / 1 待办**，无 P1 阻塞。

| #   | 缺口                                                             | 严重度                  | D6 处置建议                                                                                                                                                                                                        | 当前完成情况（2026-09-16 · 三站） |
| --- | -------------------------------------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| G1  | agent-cli wrapper 未实现（L2 核心）                                   | 高                    | D6 主体：PowerShell MVP——`workspace --create/--sync` + `task`（单模型 opencode 路径）先行；claude 路径与 `--continue` 二期                                                                                                       | ✅ **闭环**：`workspace`/`task`/`split` 已实现，C 站同套 wrapper。全链犹余站间互审拨 D7 |
| G2  | 工作区规范未实测（AGENTS.md 薄壳导入行为/claude 遮蔽/codex-memory cwd 键控在工作区场景） | 高                    | D6 V0 验证门：用 d:\RPC 自身当试点项目（AGENTS.md 已有雏形）建首个工作区实测                                                                                                                                                             | 🔵 **已覆盖/定案**：工作区机制已落地，d:\RPC 自身为首个试点 |
| G3  | ad-hoc 笔记无隔离                                                   | 中                    | wrapper `[proj:]` 前缀注入（L2-4）+ 手册纪律                                                                                                                                                                             | 🔵 **已覆盖**：任务卡/`.progress` 前缀注入已成型 |
| G4  | 大项目同步量（Textbook/Coding 全量推不现实）                                 | 中                    | `.agentsync` 默认模板 + 首同步大小预警（wrapper 检查 tar 体积, >200MB 拒绝并提示补排除清单）                                                                                                                                              | 🔵 **已覆盖/定案**：`.agentsync` 排除模板 + `--sync` 已在 wrapper 落地 |
| G5  | A 站无记忆层                                                        | ~~低~~ 配置级已解除（功能路径未测） | P1 批次 L6 已装 A 站 codex-memory（2026-09-02）；本轮实测证据=A 站 opencode.jsonc 含 plugin 条目（配置级佐证，**A 站 memory\_add\_note 实际写入未测**——D5 P1 提交称"装好即用"属自述）。路由仍以 B 站为记忆主站不变；A 站功能验证并入 G2 V0 门                                   | ✅ **闭环（统一入口实测 2026-09-16）**：三站均用 **opencode 自带 memory**（`memory.db`+`MEMORY.md`），A 亦有库（88 行）；"codex-memory 独立命令"实为误记，正主已更正 |
| G6  | 提取记忆路径（6h 闲置）未测（D5 §7-4 遗留）                                    | 低                    | 工作区日常使用自然覆盖                                                                                                                                                                                                    | ✅ **闭环**：记忆提取路径三站均可用，走 opencode 自带 memory 机制 |
| G7  | trae 派发对接（五层循环 2→3 层）                                          | 低                    | D7 范围; D6 留任务卡接口                                                                                                                                                                                               | ⏳ **保留/待办**：D6 一期 review ring 已覆盖单机，站间互审拨 D7 |
| G8  | 站上无 Mathematica / R 环境（Auto\_Prover 注入点 B/D、Cpp\_Hub R 基准对拍需要） | 中                    | **方案定案（§9.8.2）**：Mathematica→sympy 部分替代（注入点 B 单向证伪降级可用/D 基本等价，降级链 sympy→数值→Lean4→Mathematica 难例留主控站）；R 走 **CRAN noble-cran40 源装 4.6.x** 对齐主控站（apt 4.3.3 弃用），包版本 sessionInfo 对照+钉版，兰大镜像加速；执行列 D6 T0 预置（与 G9 同批） | 🔵 **主体落地（2026-09-16）**：sympy/antlr4/R 4.6.1 三站补装完成统一；**剩**：Cpp_Hub 对拍 R 依赖（forecast 等 7 个）走项目 `renv`（防版本漂移），待装 |
| G9  | 重资产预置策略（Cpp\_Hub third\_party、Auto\_Prover mathlib/.lake 缓存）   | 中                    | `.agentsync` 排除 + 站上一次性 tar 预置（不计入常规同步量）（v3 §7）                                                                                                                                                                | 🔵 **已覆盖/定案**：`.agentsync` 排除 + 站上一次 tar 预置策略已成型 |
| G10 | opencode 位置参数 bug（1.18.25 实证）上游未报/未修                           | 低                    | wrapper 已规避（stdin 管道形式）；升级窗口评估时回归验证该行为是否修复                                                                                                                                                                     | 🔵 **已定案（§9.11）**：上游至 1.18.31 仍无修复 → 维持 stdin 规避；固化"正向+负向"双用例入升级回归 |
| G11 | **并发与互斥**：同工作区双写无锁协议；llama-server 单槽下多任务排队未测（v3 §9.5）          | **高**                | **方案定案（双源合并）**：工作区级 flock（粗）+ 任务卡 readonly 字段（细，Codex RwLock 模式 §9.7-1）双层；MVP 先工作区级。Anthropic 官方纪律独立验证 + 文件集粒度细化（§9.6-2）。Bash 调用不锁（CLI 内部序列化，exec\_command=true 先例）                                            | ✅ **闭环**：工作区级互斥 + 任务卡 readonly 已落地 wrapper |
| G12 | 失败恢复：任务中断状态残留/续跑未定义（v3 §9.5）                                   | 中                    | 任务卡状态机 + wrapper 幂等回收；进 wrapper MVP 需求                                                                                                                                                                         | ✅ **闭环**：状态机 + 幂等回收 + `--continue`（≤2 次）已落地 |
| G13 | 成本/额度观测：zen 日限额无预警（v3 §9.5）                                    | 中                    | wrapper 本地 log 记调用次数；限额触发降级路径（→本地档）定义                                                                                                                                                                          | 🔵 **主体落地**：zen 仍无 API（事件驱动 OCR）；**OpenRouter** 免费档每日计数 + RPM20 硬限速 + 429 退避已落地（2026-09-16，ADR-0003） |
| G14 | 版本协同矩阵：升级回归面未清单化（v3 §9.5）                                      | 低                    | 升级窗口回归三件套：agent-cli-smoke.sh + 插件加载 + 记忆读写（与 G10 合并）                                                                                                                                                           | 🔵 **已定案（§9.11）**：版本策略 + 七项升级回归 SOP（agent-cli-smoke / G10 双用例 / provider 直连零 :4000 / timeout 注入 / 插件 / 记忆读写 / task 全链）；执行随升级触发 |
...
**agent 场景**：LLM 生成初稿（阶段 1）→ 站上编译 + ctest 回归（阶段 2 基准对齐是护城河）——**编译任务可利用 A/B 站 16 核 + 128G 统一内存（"比主控站 Win10 快"为合理假设未经 benchmark，首次试点实测确认）**；三源交叉验证策略已制度化 = 任务卡 accept 判据样板（`cmake --build && ctest` + 基准对齐脚本）。
...
| ----- | ------------------------------------------ | ---------------------- | ------ | -- | ---------------- |
| 免费·快  | `opencode/nemotron-3.5-lightning-free`（默认） | **13s**（B 站配置冒烟口径 15s） | 262k   | $0 | 日常轻任务（默认档）       |
...
值得注意的实测发现：**exec\_command=true**（shell 命令可并行）。**归因注意（v3.4 审计标注）**：源码只有声明无原因说明——"因沙箱受管/输出不落盘"是本调研的推断而非源码事实；同理"站上 opencode 的 Bash 调用天然由 CLI 内部序列化"**亦未实测**（单进程内 CLI 序列化工具调用是合理假设，但双进程并发写同目录未验证）。因此 G11 处置中"Bash 不锁"应作为**待验证假设**进 V0 验证门，而非已定事实。
...
- **升级回归里 G10 的形态（2026-09-16 修订）**：
  - **正向判据**：stdin 管道形式产出非空结果（正在用的路径必须保持）—— **这一条是判据**，FAIL 即回滚/冻结。
  - **位置参数 = 观测项，不是判据**（原为"负向断言：仍应挂死"，2026-09-16 降级）：① **证据冲突** —— 2026-09-14 复测位置参数 **4/4 成功**（仅"间歇性慢"），与本轮"位置参数无输出"相反；② **探针区分不了"挂死"与"慢"** —— `timeout N` + 尾部取值的形式下，模型慢就被读成"仍挂死"（假 PASS）；③ 官方 CLI 参考里位置参数本就是**常规用法之一** ⇒ 它"能跑"不必然是 bug 修复（原 FAIL 文案"上游行为已变"会误导）。⇒ 该断言描述的不是确定行为，按纪律"**不可把间歇现象断言为确定行为**"；改为记录"耗时/结果"供升级窗口人工比对。`agent-cli-smoke.sh` 已把该用例改为 `INFO`（不参与 PASS/FAIL，汇总行新增 `INFO=` 计数）。
...
- **G10 关闭（维持规避）**：上游无修复 → stdin 管道是唯一**受判**形态（正向判据固化进升级回归）；位置参数因"证据冲突 + 探针不可区分挂死/慢"降级为**观测项**（2026-09-16）。
...
- ~~JetBrains 2025 调研（双文件维护损耗月均 5.3h; E2 转述）~~（**v3.4 审计剔除**：无 URL 可溯源 + 专项检索零命中，按无源断言处理）
