...
## 6. 待批准 / 待实施清单（**全部已完成，2026-09-21**）

| # | 项 | 类型 | 状态 |
|---|---|---|---|
| 1 | **更正 ADR-0003 + 密钥轮换清单**里"无免费请求剩余 API"的过时结论 | 文档 | ✅ 已做（ADR-0003 加更正块；密钥轮换清单补多账户表） |
| 2 | 确认**主控用哪把 key** | 决策 | ✅ 主控与 **A** 共用旧账户（用户裁定） |
| 3 | 3 把 key 写入正本（A=旧 / B,C=新） | 配置 | ✅ 已做（B/C 新 key，格式 73B/1 行/无注释/无尾换行） |
| 4 | `secrets push` + 验收 3 个不同 `creator_user_id` | 运维 | ✅ 已做（3 个 owner；逐站指纹 MATCH+600；四端 200；门禁 PASS） |
| 5 | **`egress` 增强**（服务端权威计数 + 4 端并排 + `owner` + 双口径 + 预警 + 标注延迟；**顺带修两处现状缺陷**） | 代码 | ✅ 已做（§6.1） |
| 6 | **借用/指派机制**（方案 A：巡检→指派→台账→归还） | 代码 | ✅ 已做（`egress borrow`，§6.2） |
| 7 | 更新 [密钥轮换清单](2026-09-13_密钥轮换清单.md) 的"存放"表 | 文档 | ✅ 已做 |

### 6.1 `egress` 增强（已落地）
- `_parse_egress` 增 `owner`(`creator_user_id`) + `free_used/free_limit/free_remaining`；**更正**了 L1119 与 L1253 两处"无此 API"的过时注释
- 展示改为**双口径并列**：`free used/limit (余R, P%)` + `credits 余N`（修掉原先把 `limit_remaining` 误标为"余 N"的问题），并打 `owner` 短标签、`tier`
- 末尾给 **`N 个独立 owner`** 汇总行（一眼识别"是否已拆账"）、**⚠ 分钟级延迟**警示、本地计数**降级为旁证**并提示与"主控"行的服务端计数交叉比对

### 6.2 借用 / 归还机制（已落地并**双向自证 16/16**）
- 入口：`cluster.py egress borrow [--to A|B|C --from A|B|C | --restore A|B|C] [--go]`
  - **无参数 = 只出建议**（快照 + 到顶判定 + 给出确切命令），**不写任何凭据**
  - **`--to/--from` 不带 `--go` = 只出计划**，同样零改动（与 `flow --plan/--go`、`ttl check --go` 同习惯）
- **安全性质（逐条已测）**：① 计划路径**零改动**（正本/站上/备份三者均不变）；② 借用把被借端**自有 key 原样**存 `secrets/or-borrow/<ST>.own.key`（**不在 `stations/<ST>/` 下 ⇒ `push` 永不下发 ⇒ 站上仍只有 1 个 key 文件**）；③ **禁二次借用**（已借中拒绝）与**禁连锁**（借出方自身在借则拒绝）；④ **同账户不借**（用服务端 `creator_user_id` 判）；⑤ 借出方已无剩余则拒绝；⑥ 归还 = 拷回正本 + `secrets push <ST>`（**只推该站**，本次为此给 push 加了 `only` 参数）+ **消费掉备份**；⑦ 台账 `ops/.or-borrow.json`（已 gitignore）
- **验收（临时脚本，跑完已删；如再验按此复跑）**：B ← C 借用 ⇒ `masterB == C 的 key`、`own-backup == B 原 key`、**站上 live 也变成 C 的 key**（push 生效）、C 未被碰；二次/连锁借用被拒；`--restore B --go` ⇒ master 与 live 均回到 B 原 key、三站指纹重新互异、台账清空。**16/16 PASS**
- **一个实测踩到的实现坑（已修，值得记）**：restore 分支的 f-string 里写了**未转义的 `{file:}`** ⇒ 被当 f-string 表达式求值 ⇒ `NameError`。**教训**：f-string 里要输出字面 `{...}` 必须写 `{{...}}`；同族于项目此前"f-string 表达式段含反斜杠 ⇒ Py<3.12 不可解析"那次事故（本次已用 **PATH 的 python 3.11 复检解析**，通过）
- **边界（诚实标注）**：借用期间两站**共用同一账户额度**（故必须台账）；机制**不自动触发**（无定时器/无自动归还）—— 到顶后人按 `egress borrow` 的建议执行；**日切后需人/流程归还**（未做自动归还）
