...
| **5** | **冷启动预算** | 引擎**未加载**时起备路 | 冷启耗时 vs 备路预算（P4 的 `fallback-timeout-s` **未做** ⇒ 现与主路共享卡的 `timeout_s`）—— 冷启慢于此 ⇒ **本地备路必超时** |
...
| P3 | claude 备路**站上化**（ssh + 脚本落盘；后端按 sensitivity 分流；选站排除死锁站；fail-closed） | 代码 | 本方案主体。⚠ **前置：§3.4 选型实测** —— `public`/`sanitized` 的默认后端**未定**（"先实测再定"），先测出结论再写选后端的判据 |
