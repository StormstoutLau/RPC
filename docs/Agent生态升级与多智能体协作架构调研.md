...
| **adlc** (voodootikigod/adlc) | opencode plugin + skills + 命令面 | P5 prosecution 面: `@prosecutor-correctness/security/contract/diff/tests` 五检察官 + `@prosecutor-verifier`; `/adlc-prosecute` 扇出→核验→**loop-until-dry** (查不出新问题才放行); keyless bridge 让 LLM 门跑宿主模型 | **立场命名可借**: "prosecutor" 命名即立场 (起诉方而非协助方); loop-until-dry 有界环并入 cross-examine 协议; 不装整件 (全生命周期框架, 与本集群 spec 驱动流程重叠) |
...
| Effect 迁移: `ask()` 返回 `Effect<void>` 而非 `Promise<void>` | v1.4.7+ | 类型层面 breaking (运行时兼容) — Effect 化是持续趋势, plugin 代码须跟随 | [lancedb-opencode-pro 兼容矩阵](https://github.com/tryweb/lancedb-opencode-pro/blob/HEAD/docs/OPENCODE_COMPATIBILITY.md) |
| 跳版本升级→**插件缓存过期**: NAPI 原生依赖插件返回空模块 `{}` | v1.3.8+ | 插件加载"成功"但所有导出 undefined — 又一个静默无输出型 | [#20623](https://github.com/sst/opencode/issues/20623) |

**TUI/会话层卡死** (与插件无必然关联, 但影响长会话可靠性): 随机 TUI 冻结 busy-wait 循环 ([#12834](https://github.com/anomalyco/opencode/issues/12834), v1.1.53 起); OpenAI 流高 CPU 冻结、socket 空闲 230s 无超时 ([#29129](https://github.com/anomalyco/opencode/issues/29129)); 长思考块零流式输出、TUI 看似挂死 ([#25094](https://github.com/anomalyco/opencode/issues/25094)); Linux 长会话中段 TUI 冻结 ([#35641](https://github.com/anomalyco/opencode/issues/35641))。**高相关项**: 首次运行 ripgrep 下载停滞→grep/skill 工具**无限挂起零报错** ([#23891](https://github.com/anomalyco/opencode/issues/23891)) — 两站在中国网络环境访问 GitHub Releases 正是高危场景, **必须预装系统 ripgrep**。

#### 8.6.2 claude code: marketplace 机制自身的破坏性故障

| 事件 | 故障模式 | 证据 |
| --- | --- | --- |
| **marketplace 自动更新静默清空插件**: staging→production 换页不完成, manifest 丢失→下次启动"对账"删掉插件目录; `--channels` 横幅谎称 "Listening" 但进程从未启动; 手工恢复无效(再次启动又被清) | telegram 插件会话中途静默死亡, 无错误无日志无恢复机制; 同类 [#40153](https://github.com/anthropics/claude-code/issues/40153) 是**整个 marketplace 目录被删** | [#41701](https://github.com/anthropics/claude-code/issues/41701) (v2.1.87) |
