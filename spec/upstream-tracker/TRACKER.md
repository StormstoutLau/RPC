...
| **EV-3** | **CIRU StrixLink 双机部署评估**（`jcbtc/GLM5.3-Flash-CIRU-STRIX-IU4`）—— **GLM-5.3-Flash 320B、TP=2、走 vLLM/ROCm 10 绕开 glm5next 上游依赖** | 引擎换路线 | ✅ **已取证（E1，2026-09-16）**：双机 USB4 + gfx1151 + 各 128 GiB；权重**已按 `rank-0`/`rank-1` 切好**（各 ~**83.25 GiB**）；`INSTALL-RUNTIME.sh` 自包含；实测 **decode 23.686 t/s / 402.5 prompt t/s**（64K+NHI+DFlash2）；**⇒ 不可单机跑（必须两机）** | 中高（两机新栈 + 磁盘 + 治理接入） | ① 三站 NVMe 余量；② 内核 **NHI/USB4STREAM**（否则退回 RCCL ≈341 µs）；③ ADR-0004 立项 | [调研 §2](../../docs/research/2026-09-16_分布式推理路线可行性调研_CIRU-Skulk-多机TP.md) |
