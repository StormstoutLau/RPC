...
status: draft
date: 2026-09-02
depends: [Agent生态升级与多智能体协作架构调研 (docs/, 2026-09-02 含 §7.6 排幻觉审计版)]
...
> **Spec 步骤**: Step 3-4
> **基于调研**: [Agent生态升级与多智能体协作架构调研.md](../../docs/Agent生态升级与多智能体协作架构调研.md)（含 §7.6 审计，本文所有决策仅引用审计后结论）
...
| ars-academic-pipeline | A | 同上（§7.5③） |
| paper-lookup | B（REST 多数免 key） | P1 实测连通后定（§7.5②） |
| **合计** | 10-11 件 | 全部 A 级除 paper-lookup |
...
| **T1 升级** | B 站 opencode 1.18.9→1.18.25（npm 平台包路径，D4 已验证的备选法） | `opencode --version` + PONG 冒烟（调研 §1.1 同款） | — |
...
| **T5 记忆试点** | B 站装 codex-memry（钉版本）→ 正常使用一个会话 → 闲置 6h 触发提取 | 次日新会话问"上次这个 repo 做了什么"，应能引用前日内容；检查 memories/ 目录有 markdown 产出 | T1 |
...
| A4 | 定制技能可用 | 10 件在两站 `/` 列表可见；抽验 3 件触发出结构化输出（六层推理框架/what-if 分支表/research 卡片） |
...
| 溯源 | 本设计每个决策的依据锚定在调研报告 §2.2/§6/§7.5/§7.6；后续引用本文时须连同调研锚点一起引 |
