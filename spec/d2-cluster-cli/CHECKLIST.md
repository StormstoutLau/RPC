...
| RESEARCH §3.3 路由表 → IMPL §3.1 常量层 | ☑ | ROUTE 3 行 + RPC_MODELS 集合一致 |
...
| 未越 D1 边界（不做 watchdog/fallbacks） | ☑ | |
| 未越"不做第四个面板"边界（静态快照非常驻） | ☑ | |
...
| RPC 类模型集合 | §3.3 | §3.1 RPC_MODELS | §3 #5 | ☑ |
...
| # | 验收项（IMPL §5） | 验收命令 | 通过 |
|---|------------------|---------|------|
| 1 | status 聚合 | `python ops/cluster.py status` | ☐ |
| 2 | 单站宕机容错 | 停 A 站 llama 后 status | ☐ |
| 3 | load 路由-A | `cluster.py load gpt-oss-120b` → A 站 READY | ☐ |
| 4 | load 路由-B | `cluster.py load nvidia-nemotron` → B 站 READY | ☐ |
| 5 | RPC 类拦截 | `cluster.py load deepseek` → exit 2 + 手动步骤 | ☐ |
| 6 | e2e 双路由 | `cluster.py e2e` → exit 0（未加载时 exit 3 + 提示） | ☐ |
| 7 | HTML 快照 | `cluster.py status --html` → 文件生成、浏览器可开、4 链接可达 | ☐ |
| 8 | 手册更新 | docs/手册 §2.4 增补 | ☐ |
| 9 | 连续 load 幂等（P1 回归） | 已加载 gpt-oss 再 `load nvidia-nemotron`（或反向）：应先 unload→GTT 释放→新 load 成功，无并发加载 | ☐ |
