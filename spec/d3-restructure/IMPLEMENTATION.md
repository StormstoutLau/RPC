...
- `双机推理集群使用手册.md.bak.20260901`、`llama.cpp-0.2.0.tar.gz`、`downloads/` 均在 .gitignore 内未入库，保留本地
  - **⚠ 2026-09-22 加注（本条"保留本地"的决定已被取代；**不改写上文，只加注**）**：前两项经取证**已清理**，`downloads/` 未动。**为什么必须先取证**：D3 那轮裁定过"保留本地" ⇒ 清理 = **推翻旧决定** ⇒ 不能只凭"看像垃圾"就删（且它们被 gitignore ⇒ **git 无兜底**）。取证结论：**`双机…bak`** 内容 = git blob `992e5e48ce1ce5b6b8e0cce5e1dda34b3f59b237`，位于可达历史 `612064f^:双机推理集群使用手册.md`（还原后 **523 行，与原文件逐行一致**）⇒ **零信息损失**；**`llama.cpp-0.2.0.tar.gz`** 内容 = 上游 tag **`v0.2.0`**（[TRACKER 09-17](../../spec/upstream-tracker/TRACKER.md) 已判定"上游 tag 可得"，且 B 站同类副本已于 09-17 清除）。⇒ 仓库根由此只剩 `.gitignore` 与 `LICENSE`。**复原处方**：`git show 612064f^:双机推理集群使用手册.md > 双机推理集群使用手册.md.bak.20260901`。
