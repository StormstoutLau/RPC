...
| H3: gptoss\_spec\_test2.sh 疑含未知内容            | 补核后消除疑虑  | 实测纯 CRLF 差异，核对门关闭                   |
...
| 2  | gptoss\_spec\_test2.sh 无内容变更 | `git diff gptoss_spec_test2.sh`（输出仅 LF/CRLF warning） | ☑                     |
| 3  | B 站 cron 两条 bjork\_deepfix   | `crontab -l`（B 站）                                    | ☑ 原文留档于 RESEARCH §3.6 |
...
| -- | -------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| T1 | #1 工作区干净（提交后）+ #7 远端同步     | `git status --porcelain` / `git log origin/main -1` | ☑ commit `1ef4179`+`ccb0c28`+`17f121a` 已 push，工作区干净                            |
| T2 | #2 /tmp 无 key              | 两站 `grep -rl 'sk-RPC-' /tmp/`                       | ☑ B 站 4 文件 + A 站 2 文件删除，两站零命中（终验 B_FINAL_CLEAN / A_STATION_CLEAN）              |
| T3 | #3 txt 已删                  | `Test-Path d:\RPC\新建文本文档.txt`                       | ☑ commit `17f121a`，Test-Path False                                                  |
| T5 | #6 bak 收敛 ≤1/站             | `ls ~/.config/opencode/*.bak*`                      | ☑ bak≡bak2（diff IDENTICAL）删除后 B 站剩 bak3、A 站剩 bak-0910                                |
| T6 | #9 前置断言 → #8 crontab 清空    | `crontab -l`（清理前后各一次）                               | ☑ 2026-09-01 22:09 执行：断言通过（仅两条 bjork 条目）→ 备份 `~/crontab.bak-20260901`(385B) → `crontab -r` → 验证 no crontab |
| T4 | #4 版本 1.18.25 + #5 PONG 冒烟 | `--version` / `opencode run ... 'reply PONG'`       | ☑ 官方 upgrade 失败（github 不可达 exit 28）→ npm 平台包备选成功（`opencode-linux-x64@1.18.25` 60MB tarball 提取 install）；`--version`=1.18.25；PONG 冒烟通过（stdin 需 </dev/null）；旧二进制存 `~/.opencode/bin/opencode-1.18.9.bak` |
...
| 实施验收（Trae 执行） | T1-T6 全部完成，9 项验收全过（§3）；T4 走 npm 备选路径达成 | 2026-09-01 22:30 |
| 用户终审        | ☐ 待签                  | —            |
...
| 2026-09-01 | v1.2：实施完成，§3 全部勾销（T4 记录 npm 备选路径：官方 upgrade 因 github 不可达失败，npm registry 直连可用） |
