...
#### 8.7.6 跨站扇出 L1 实机回填（2026-09-04，§8.7.4 判据执行）

> **方法注记**：两站 unsloth 8080 均仅监听 `127.0.0.1`，跨站不可直连端口；采用**无侵入 SSH 隧道**（不重启/不改加载参数/OOM 零风险）——B 站 `ssh -NL 127.0.0.1:18081:127.0.0.1:8080 scott-lau@10.10.10.1` 把 A:8080 映射回 B 本地 18081。编排层在 B 站并发打「B 自己 8080 + 隧道 18081(A)」。判据沿用墙钟收敛（无 `/properties`，`engine_stats.running` 不可用，见 §8.7.5）。脚本：~~`ops/station-bin/_bs2_cross.py`~~ **(2026-09-15: 已按 ADR-0004 清减删除; 隧道法已被 `agent-cli --RemoteHost <站>` + 站内自发现端口取代 —— 本段为 2026-09-04 历史记档)**。

**实测（gpt-oss-120b-MXFP4，两站同权，REQS=4）**：

- **A 串行基线**：4 次顺序打到 A（隧道）：1.5 + 1.7 + 1.9 + 1.7 = **6.8s**（avg 1.7s），全 200。
- **跨站扇出**：A/B 交替各 2 并发 → 每请求 dt≈4.4–4.8s，**cross\_wall=4.8s**。

**判据回填（§8.7.4 先写死）**：

| 判据 | 期望 | 实测 | 判定 |
| ---- | ---- | ---- | -- |
| cross ≤ A串行×~1.6 | ≤ 10.9s | 4.8s（ratio 0.71） | ✅ |
| 数据面真并行 | cross ≪ A串行sum（≈max） | 4.8s < 6.8s，但 > max(1.9s)×2.59 | ⚠️ 部分 |

**结论**：
- **跨站扇出成立**：cross 4.8s ≤ A 串行 6.8s×1.6，**~1.4× 数据面收益**（6.8/4.8），证 A+B 两独立实例确实分担了负载。
- **同站内并发仍被统一内存带宽/槽位顶起**：并发时各站单请求 1.7s→4.8s（2 并发抢带宽，单请求变慢 2.8×）→ 与 §8.4「同站并发收益被槽位/带宽封顶」一致。收益来自**跨站分摊**而非同站多槽。
- **工程落地意义**：编排层扇出优先**跨站各 1 并发**（而非同站多并发），单请求不至于因带宽竞争而恶化；ADR-0002 决策 C「编排层并发 HTTP 直连 B/A 两站 8080」实证可行，跨站链路已闭环。

**跨站门判定**：✅ **通过**（判据 cross≤1.6 达标；观测到 §8.4 带宽封顶一致的同站现象，未破坏跨站收益）。
