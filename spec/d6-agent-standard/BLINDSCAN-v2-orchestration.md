...
> **方法注记**：两站 unsloth 8080 均仅监听 `127.0.0.1`，跨站不可直连端口；采用**无侵入 SSH 隧道**（不重启/不改加载参数/OOM 零风险）——B 站 `ssh -NL 127.0.0.1:18081:127.0.0.1:8080 scott-lau@10.10.10.1` 把 A:8080 映射回 B 本地 18081。编排层在 B 站并发打「B 自己 8080 + 隧道 18081(A)」。判据沿用墙钟收敛（无 `/properties`，`engine_stats.running` 不可用，见 §8.7.5）。脚本：`ops/station-bin/_bs2_cross.py`。
