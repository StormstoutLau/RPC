...
- Write-RunJson（**实际实现为内联**——见 §3.1 两处构造点：`Invoke-Task` / `Invoke-Task-Claude`）→ 落**主控站** `<proj>/agent-out/<ts>/.agent-run.json`（S4 容错）。**契约哈希三字段（M1，不变式 5 落地）**：主控站计算 `prompt_sha256`（消毒后终版 prompt）+ `attach` + 远端回收后计算 `content_digest`（agent 输出全文 sha256）——三字段任一为空即验收失败（A11 判据含此项）。
  ⚠ **两处已订正（2026-09-16；① 于 2026-09-18 再度更新）**：① `attach`：**2026-09-18 起补哈希**（[ADR-0007](../../adr/ADR-0007-证据流阶段推进路线与改动验证闭环.md) 缺口 5）—— **有附件的新 run** 落**对象数组** `{name, src, kind: file\|dir, files, sha256}`（摘要源于**站上** `attach-manifest.txt` 的逐文件 `sha256sum`，由 console 单一实现点计算 ⇒ 落进 run.json 后**被链钉住**）；**老 run 与 claude 备路仍为名字数组**（本次刻意不做备路）；② **ADR-0005 另增** `accept_golden.sha256`/`base` 两个契约键，并把 6 类证据件收进 runDir（`judgment-record.txt`/`progress-trace.txt`/`prompt.txt`/`accept-cmds.txt`/`golden-cmd.txt` + claude 备路 `stderr.txt`）；**ADR-0007 再增** `workspace-diff.txt`（缺口 4）、`attach-manifest.txt`（缺口 5）与 `session-meta.txt`（缺口 8）——完整清单见 [ARCHITECTURE.md](ARCHITECTURE.md) §6 表
  - **卡身份（ADR-0007 前置，2026-09-18）**：run.json 增 `card{path,sha256,bytes,front_matter}`，卡的**原始字节**归档为 runDir **`card.md`**（`Get-CardIdentity` 为**单一实现点**，主路 + claude 备路共用）。**为什么**：卡是**最大的注入物**（它决定 prompt/验收/golden/manifest 本身），此前 run.json 与站上 `.meta` 都不记卡 ⇒ "那次跑的是哪张卡的哪个版本"不可判；且**卡会改**（同日实测改了 5 张夹具卡）⇒ 只记 hash 无法复跑，必须归档卡字节（`prompt.txt` 只有正文、无 front-matter）。**护栏**：无 front-matter 卡会静默退化为 `readonly=false` + `sensitivity=public` + **无 accept-golden/manifest**（实测：agent 跑偏仍 rc=0、无人判 ⇒ **验收本身消失**）⇒ 要求显式 `-Sensitivity` 才放行，否则 `REJECT no-front-matter-card (exit 2)`
  - **遥测来源（缺口 8，2026-09-18）**：`session_id` / `usage` / `timestamp_start|end` **不再是占位** —— 由站上 [`_oc_session_meta.sh`](../../ops/station-bin/_oc_session_meta.sh) 从 **opencode 会话库**取该次会话的聚合（`tokens_*`/`cost`/时间戳/`tool` part 计数）后由 console 落键；**取不到则显式 `usage.source=unavailable`**（headless stdout 不吐 usage 的既有结论不变 ⇒ **不从输出反推、不猜数**）。口径与上限见 [ADR-0007](../../adr/ADR-0007-证据流阶段推进路线与改动验证闭环.md)「缺口 8 落地与实测」
- **--attach 传输（M4）**：attach 文件随 Invoke-Task 前置的附加 sync 传站。⚠ **实测实现与本节 v1 声明多处不同（2026-09-18 订正）**：实际是 `scp` **直传工作区 `.attach/<name>`**（**无** tar 打包、**无** `.attach/<ts>/` 子目录、**无** 50MB 限制），prompt 追加附件名清单；且 **`.attach/` 从不回收**（实测站上残留 09-05/09-12 五次派发的附件 —— 与"结束即回收、工作区不留副本"**正相反**）⇒ 已改为**派发前 `rm -rf` 清空**（**无条件**执行，含"本次无附件"的情形 —— 实测无附件派发会留着上一轮附件；等价于所声明的"单次 task 生命周期"，且不依赖 collect）。附件身份另由 runDir `attach-manifest.txt` + run.json `attach[].sha256` 记录（缺口 5）
- Invoke-Collect：out/ tar+scp 拉回 `<proj>\agent-out\<ts>\`
- 台账 agent-runs.log 追加（CSV 一行：ts,proj,model,sens,exit_code,**queue_s,run_s** —— 后两列顺序 **2026-09-18 实测订正**：实现写的是 queue 在前、run 在后（本轮 e2e 实测 `…,0,2,56` 与 `.meta` 的 `QUEUE_S=2/RUN_S=56` 对齐），`cluster.py` 的解析亦按此顺序）
