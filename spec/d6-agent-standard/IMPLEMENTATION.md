...
- .agentsync 四型模板落盘（python: `__pycache__/ .venv *.egg-info raw_md/ new_papers/ *.duckdb`；cpp: `build/ third_party/ *.o`；doc: 空；lean4: `.lake/`）
- 验收：A7
...
- Write-RunJson：DESIGN §6.2 schema 落 out/.agent-run.json（S4 容错）
...
| A5 | V0-5 Bash 不锁 | 双 ssh 并行 `echo \| opencode run '写/tmp/a.txt与b.txt'` → 两文件皆成且无损坏 |
...
| A11 | 端到端本地 | `task paper --card test-cards/echo.md --model nemotron` → 退出 0 + .agent-run.json 契约字段齐 + agent-out\<ts>\ 有产物 |
| A12 | 免费档契约 | 同卡 `--model lightning` → .agent-run.json model 字段=opencode/... + 台账一行 |
| A13 | 错误注入 | timeout_s: 5 卡死循环任务 → 退出码 6 + state=failed{timeout}；断 ssh（拔线模拟或杀 sshd 连接）→ 重试 1 次语义 |
