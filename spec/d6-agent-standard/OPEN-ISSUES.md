...
| O-52 | 证据/采集面 | P2 | **A-② 采集射程只支持"单文件扁平名"**（`Test-EvmStatePull` 规则⑤：`path` 禁 `/`）⇒ 产物名不可知（glob）/ 需目录结构时取不回 | ⏳ 待立项（**O-45 调研的替代方案①**）：补 **glob**（`out/*.json`），**仍不执行用户命令** —— 远端**固定枚举命令** + **纯函数校验 pattern**（仅 `[A-Za-z0-9._*-]`、须 `out/` 前缀、禁 `..`/绝对/空白/shell 元字符），**不拼接**。判据同 `Test-EvmStatePull`：离线正反夹测 | O-45 · O-40 |
