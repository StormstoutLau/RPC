...
| O-43 | 通道/归因 | **P1** | **zen（`opencode/*`）"需要 tty"被误判为"缺凭据"**（2026-09-21 结论）：非 tty 挂死 `RC=124` 且 DEBUG 日志**无任何 401/403/凭据错误**；套 `script -qec` **伪 tty** 后**同一模型 43s 返回 `ZEN_OK`** ⇒ **恢复 zen 不需要登录**，门槛是"给非交互路径伪 tty + **PTY 输出净化**" | **✅ 机制已验通 + ✅ 采样结论：暂不切回**（zen 5 次 **1/5 成功**，唯一成功 92s，其后**连续 4 次 152s 零输出** ⇒ 呈**限流**特征；对照 openrouter **2/2 成功、19–25s**） | D6-P2 · ROUTE_TABLE |
...
#### O-41：B2 报出的**两条"疑似未登记假绿"**（待复核）

> 来源：B2（门禁假绿审计）产物 `out/falsegreen.md`（17 项断言全覆盖、含每项**最小反例**）。

| # | 断言 | 报出的假绿条件 | 我的初判 |
|---|---|---|---|
| 1 | `doclinks` | 对**绝对路径 / 盘符 / URL / 占位词一律"不可判"跳过** ⇒ 写一个**不存在的绝对路径**也能 PASS | **疑似未登记**（待复核台账） |
| 2 | `engine` | **残留阈值 2048MB 硬编码** ⇒ 一个占 1.5GB 且不监听任何 `ENGINE_PORTS` 的残留进程会被判"正常" | **疑似未登记**（待复核） |
...
**⇒ 裁定建议：暂不切回 `ROUTE_TABLE`**（证据：20% 成功率 + 失败形态是**静默挂死**）。
经济账也不站在 zen 这边：zen 的卖点是"省 openrouter 免费配额"，但 20% 成功率意味着**平均要投 5 次**，
而 openrouter 免费档本身**零付费**且 100% 成功 ⇒ **换成 zen 反而更贵（时间）且更难诊断**。

**若将来仍要用 zen（可选下一步实验）**：做**间隔投放**（例如每 5 分钟 1 次，共 4 次）——
用以区分"**频率限流**（有可用窗口）"与"**额度/服务受限**（无窗口）"；这决定它是否还有被使用的可能。
