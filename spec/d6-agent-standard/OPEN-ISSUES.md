...
| **G14 / G10 升级回归矩阵** | **方案已定（§9.11 七项 SOP + 版本策略）**；G10 判定维持 stdin 规避。**执行**随升级触发 | 升级窗口开启 |
...
| **`infer-load` 把 API key 明文写进站上日志** | **09-16 新登记（凭据面卫生缺陷）**：[infer-load](../../ops/station-bin/infer-load) 就绪后执行 `log "API Key: $UN_KEY"` ⇒ 明文 `sk-unsloth-<32hex>` 落 `~/.unsloth/run-<alias>.log`。与框架"脱敏/明文面收敛"纪律不符（09-14 曾专项清理 13 处明文）。**评估**：影响面有限（站内 700 目录、key 每轮加载重铸、仅绑 127.0.0.1），但属可修项 | 待修：日志只写长度或掩码（如 `sk-unsloth-****`），保留"已落盘/未落盘"可观测性 |
| **`secrets push` 会用正本陈旧 key 覆盖站上真 key** | **09-16 新登记（运维陷阱）**：`cluster.py secrets` 只有 `status\|scan\|push`（**单向下发、无回写**），而 `~/.config/rpc/unsloth.key` 是 `infer-load` **每次加载重铸**的站内产物 ⇒ 若在站上加载后直接 `push`，正本里的旧 key 会**覆盖站上真 key**，打断该站 opencode/claude。本次已按正确顺序执行（先回写正本再 push），并在 [ADR-0003](../../adr/ADR-0003-OpenRouter密钥与egress路由管理.md) 与[轮换清单](../../docs/security/2026-09-13_密钥轮换清单.md) 写入"push 前必须回写"约束 | 人工纪律兜住中；可考虑加门禁断言（"站上 `unsloth.key` ≠ 同站正本 ⇒ WARN"），或给 `secrets` 增 `pull` 动作 |

> **台账维护规则回归**: 本表是 D6 未决问题唯一总账；上表"真实剩余 open"**10 项**（其中 5 项为 2026-09-16 新登记：`_agent-cli-bom.ps1`、G10 负向用例 flaky、`cpphub-001` 卡与 PROJECTS 映射不一致、`infer-load` 日志明文 key、`secrets push` 正本覆盖站上真 key）之外均为已闭环/已覆盖，若未来 OPEN-ISSUES 快照提到"G 清单未闭合"应能溯源到这 10 项 + 对应 G 编号。（另：同日已处置的 Cpp_Hub 假子模块/unmapped gitlink 与 claude key 单一真值修正，均属**已闭环**，见 [DEVELOPMENT-LOG 2026-09-16 ⑧⑨](DEVELOPMENT-LOG.md)）
