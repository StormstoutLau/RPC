...
| **证据流可重放性（evidence_manifest + 异基座管线审计）** | **阶段 0 已执行（2026-09-16，[ADR-0005](../../adr/ADR-0005-任务卡证据回收闭环.md)）**：证据回收闭环落地 —— `.meta`/`.prompt.txt`/`.progress` 原文/`.accept-cmds.txt`/`.golden-cmd.txt` 归 `agent-out/<ts>/`，`run.json.accept_golden` 增 `sha256`/`base`，TEMP 泄漏与失败路径一并修；**三项自证能力实测 PASS**（`sha256(prompt.txt)==prompt_sha256`、`accept_golden.sha256==仓库源哈希`、`judgment-record ↔ run.json` 逐项一致），collect 115s→49s。**剩阶段 1（manifest 规范化）/ 2（离线复验器）/ 3（异基座管线审计）** 未做，方案见[调研文档](../../docs/research/2026-09-16_任务卡证据流可重放性调研.md) §7.1 | 待决策：继续阶段 1 或先评估 `evidence-manifest` 字段草案 |
| **框架级：每次 ssh/scp 建连 14-17s** | **09-16 新登记（性能）**：5 次采样证实（`AddressFamily=inet` 仅省 3s、与 GSSAPI 无关；**ControlMaster 在 Win32-OpenSSH 9.5p1 不可用**：`getsockname failed: Not a socket`）⇒ **所有** ssh/scp 调用点都付这笔钱（实测单次 task run 墙钟 ≈ 420s，其中模型运行仅 21-53s）。本轮已用"合批回收"把 collect 连接数 8→4（省 66s/run），但 station-ready / slot-gate / profile / sync / lock 等步骤仍各付一次 | 待定（性能专项）：查 sshd 侧（如 `UseDNS`）/ 换连接方式 / 进一步合批 |
...
> **计数说明（2026-09-16 晚）**："真实剩余 open" 现为 **9 项** —— 上表 5 项长期项（O-07/G13、G14/G10、G7/O-16、G8 R 依赖包、记忆协同设计取舍）+ 当日新登记 4 项（root 级脚本治理空白、证据流可重放性阶段 1-3、框架级 ssh/scp 建连开销、`Get-FrontMatter` 不剥引号）。当日曾登记的另 5 项（`_agent-cli-bom.ps1`、G10 负例 flaky、`cpphub-001` 映射、`infer-load` key 明文、`secrets push` 覆盖）**全部当日闭环**，均保留划除行以便溯源。同日已闭环的更大项另有：Cpp_Hub 假子模块/unmapped gitlink、claude key 单一真值修正、studio 日志明文面收敛、凭据下发方向保护、**任务卡证据回收闭环（ADR-0005 阶段 0）**（见 [DEVELOPMENT-LOG 2026-09-16 ⑧⑨⑩⑪⑫⑬](DEVELOPMENT-LOG.md)）。

> **台账维护规则回归**: 本表是 D6 未决问题唯一总账；上表"真实剩余 open"之外均为已闭环/已覆盖（数量与逐项溯源见上方**计数说明**），若未来 OPEN-ISSUES 快照提到"G 清单未闭合"应能溯源到那 9 项 + 对应 G 编号。
