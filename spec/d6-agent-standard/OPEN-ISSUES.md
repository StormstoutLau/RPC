...
> **台账维护规则回归**: 本表是 D6 未决问题唯一总账；上表"真实剩余 open"**9 项**（含 2026-09-16 新登记的 4 项：`_agent-cli-bom.ps1`、G10 负向用例 flaky、`cpphub-001` 卡与 PROJECTS 映射不一致、`secrets push` 正本覆盖站上真 key；同日新登记的第 5 项"`infer-load` key 明文"已**当日闭环**，保留划除行以便溯源）之外均为已闭环/已覆盖，若未来 OPEN-ISSUES 快照提到"G 清单未闭合"应能溯源到这 9 项 + 对应 G 编号。（另：同日已处置的 Cpp_Hub 假子模块/unmapped gitlink、claude key 单一真值修正、studio 日志明文面收敛，均属**已闭环**，见 [DEVELOPMENT-LOG 2026-09-16 ⑧⑨⑩](DEVELOPMENT-LOG.md)）
