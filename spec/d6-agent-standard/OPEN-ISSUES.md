...
| O-50 | 文档/一致性 | P2 | **手册 §1.3 的"真实剩余 open = 4 项"枚举已过时**（其后新增 O-30/O-33/O-35/O-39/O-45/O-47/O-48/O-49/O-51/O-52）⇒ 两处各持一份枚举 | ◐ **本轮就地标注过时 + 校正计数**（门禁 15→18 项；gap 16→28；§2.4 同步）；**彻底取消手册第二份枚举** 留 D6-P1-2 | D6-P1-2 |
...
| ~~O-52~~ | 证据/采集面 | P2 | **A-② 只支持"单文件扁平名"** ⇒ 产物名不可知时取不回 | **✅ 机制已实施 + 端到端复跑（2026-09-24）· ⚠ 但实测暴露出【适用性限制】**<br>① **真 bug（已修）**：`ls` 的 pattern **不能加引号**（引号把 `*` 变字面量 ⇒ 永远 0 匹配）；护栏已钉住"未加引号"不变量<br>② ★ **适用性限制（真实、未掩盖）**：`out/` 是**累积**的 —— 实测同目录下有 `glob-probe.txt`/`pilot-arm-O.txt`/`pilot-arm-Z.txt`/`smoke.txt` **4 个 `.txt`** ⇒ **"恰好 1 个"的规则在真实工作区里几乎必然拒绝**。⇒ 该设计**只在"干净工作区"下可用**；要用须配"产物隔离"或更严的文件名模式（另案） | O-45 · O-40 |
...
| O-54 | 文档/一致性 | P2 | **受理状态机副本收敛后仍余 4 处**（按 [路线总表 §11.1-B](../../docs/2026-09-23_D6-D7分阶段执行方案.md) 实测 9 处，M-5/M-6 只覆盖了 代码白名单×2 + `_INBOX_NEXT` + `_ACTION/_ACTIVE` + README §3） | ◐ **M-5/M-6 已收敛主体 + 加对账断言（2026-09-24）**：真值源 → [`inventory/inbox.yaml`](../../inventory/inbox.yaml)，两处代码改消费它，门禁 `mirror` 对账 README §3（先验红已验）。**余 4 处显式登记**：① 前端 JS `inboxBadge` 硬编码（`cluster_web.py`）② `rpc_check.py` `check_inbox` 状态子集又枚举（业务规则集）③ `cluster.py` `seal` 提示语"交付态三元组" ④ **手册 §1.3:94-96「唯一对外声明版」**（**已漏 `rejected`**）⇒ ④ 是**唯一对外契约**，优先处理 | D6-P1-2 后续 |
