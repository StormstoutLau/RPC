...
| **卡片身份不进证据（run.json/.meta 无 card 路径与哈希）** | **09-18 新登记（证据完整性；四项目实测发现）**：为 3-b-2 定案做四项目画像时实测 —— `.meta` 全文 grep **无 `card`**；run.json **无 card 路径、无 card 哈希**（只有 prompt 正文 + `evidence_manifest`）⇒ **"那次跑的是哪张卡的哪个版本"不可判**、复跑无法定版。**为什么比缺口 5（attach 哈希）更基础**：**卡是最大的注入物**（决定 prompt / 验收 / golden / manifest 本身）。**实测代价**：4 个项目**两种卡格式并列** —— Cpp_Hub 用 `F:\Cpp_Hub\dispatch\TASK_*.md`（**无 front-matter**，手工任务书；复现关键 `commit=b278151` **只存在于 prompt 文本**），RPC 用 front-matter 卡 ⇒ 同一框架下"卡"的形态不齐，审计题面无法把"卡"当一等对象 | 待办（**建议排在 3-b-2 之前**）：run.json 增 `card{path, sha256}`（与缺口 5 同法 —— 落进 run.json ⇒ **自动被链钉住**）；需先定"无 front-matter 任务书"的处置（拒绝 / 降级标 `card-less`）；属 `.ps1` 改动 ⇒ 走阶段 0.5 夹具 e2e |
