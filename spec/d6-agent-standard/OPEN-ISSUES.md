...
| **⚠ `local-only` 闸不覆盖 claude 备路 ⇒ 敏感卡可实际出网（2026-09-21 新登记，安全策略洞）** | **背景**：`sensitivity: local-only` 的硬闸有三处（`Resolve-Model` L435 / `Invoke-Task` L987 / 路由 cmd L1837），判据一律是 `local-only` + `^opencode/` ⇒ `REJECT … exit 4`（无覆写通道）。**洞**：**`Invoke-Task-Claude` 没有任何 sensitivity 判据**，只校验"型号在表内 + `station=''`"；而 `AUTO_FALLBACK` 调用点（L1763）也不判 sensitivity。**两条实际出网路径**：① 卡写 `sensitivity: local-only` + `cli: claude` ⇒ L987 放行（型号非 `opencode/*`）⇒ `Invoke-Task-Claude` ⇒ **OpenRouter 云端**；② `local-only` 卡 + 站上本地型号 ⇒ 站上引擎死锁 `rc=6` ⇒ `AUTO_FALLBACK` ⇒ `Invoke-Task-Claude -model claude` ⇒ **云端**。**为何现在才严重**：此前该路过 `api.anthropic.com` 且**不可用**（`Not logged in`）⇒ 洞是**惰性**的；2026-09-21 免登录 + OpenRouter + 型号重映射把备路**修成真能用** ⇒ **洞变活**。**破的是** [DESIGN.md §358](../../spec/d6-agent-standard/DESIGN.md) 的不变式（"local-only 的 prompt 字节永不离开主控站→站内本地模型路径"）。**影响面**：`test-cards/` 里 **20/25 张是 `local-only`**。**另注**：ZDR **不能**兜底 —— OpenRouter 官方明确"ZDR 只保证 provider 不留存，数据仍会到达 provider 并被模型处理，且不覆盖处理地"⇒ `local-only` 要的是**不出网** | ✅ **P0 止血已实施并双向自证（2026-09-21，剩 P2–P5）**：新增**唯一判据** `Test-SensitivityEgressAllowed`（L905，纯函数：`local-only` × 后端出网性）并在**两个出网入口各判** —— `AUTO_FALLBACK` 调用点（L1772，**拒绝兜底** ⇒ fail-closed）+ `Invoke-Task-Claude`（L2039）。**出网侧证据**：① 夹具 60/60（真值表 5 例 + "两处调用点在"覆盖断言）；② 实弹探针 `_probe_fallback.ps1` 驱**真实** `Invoke-Task`，两条路径均 `rc=4` 且 **claude run 计数不增**（= 未发出请求）；③ **变异自证**：把判据短路成 `return $true` ⇒ 两条路径**各真跑出一个 claude run**（真发出境请求）且探针 FAIL ⇒ 证明**洞是真的、探针不是结构性失明**。**仍未闭合**：[P2 判据升级为"后端 `egress` 属性"](../../docs/research/2026-09-21_D6备路站上化与sensitivity设闸调研与方案.md)（现在两处 `$backendEgress` 是**硬编码 $true**）、**P3 claude 备路站上化**（站上 claude 现指 `127.0.0.1:8080` 本地引擎 ⇒ `local-only` 天然不出网；且同时绕开主控 Trae 沙箱）、选站排除死锁站、**全站不健康 ⇒ fail-closed**。**注意**：P0 后 `local-only` 卡若主路死锁 ⇒ 直接 `rc=4` **不再兜底**（刻意让策略拒绝盖过超时，理由见代码注释） |
