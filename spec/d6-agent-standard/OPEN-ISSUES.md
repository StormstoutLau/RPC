...
| O-52 | 证据/采集面 | P2 | **A-② 采集射程只支持"单文件扁平名"**（`Test-EvmStatePull` 规则⑤：`path` 禁 `/`）⇒ 产物名不可知（glob）/ 需目录结构时取不回 | **✅ 机制已实施（2026-09-24）· ⏳ 端到端待跑（如实标注，不谎报"已验证"）**：采用**最小安全设计** —— **不引入多文件落盘、不动链的 `<name>.txt` 约定、只在 `Test-EvmStatePull` 后加一个分支**（既有"具体路径"行为**零改动**）⇒ 缺口 (a)「产物名事先不可知」被吃掉；(b) 目录结构 / (c) 站上加工 **仍未覆盖**（如实留白）。四条不变量：① **字符集白名单** `^out/[A-Za-z0-9._*?-]+$`（挡 `'`/`;`/`$` 等元字符 —— pattern 要送到站上）② 远端**固定枚举** `ls -1 -d -- '<pattern>'`（**单引号包裹、只作参数、不拼接**）③ **恰好 1 匹配才收**（0/>1 拒，**不猜** —— 猜一个会在证据链上留错件）④ ★ 解析出的名字**来自站上 FS ⇒ 按不可信输入复校验一遍**且不得再含通配。**静态护栏 15 条**（`test_cli_concurrency_guards.py`）+ PS 语法 + 全量 9/9 ✅；**未跑真实派发** | O-45 · O-40 |
