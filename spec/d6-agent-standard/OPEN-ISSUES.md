...
| **`syntax` 断言抓不到"无 BOM 的含中文 .ps1"（本轮新发现）** | **09-18 新登记（判据盲区）**：`ops/station-bin/_fm_golden_test.ps1`（**卡解析器的回归守门夹具**）**缺 UTF-8 BOM**（首字节 `23 20 3D`）。PS5.1 按 ANSI 解码含中文的脚本时，**3 字节 UTF-8 序列被当 2 字节 GBK 解码会吞掉相邻换行** ⇒ 语句被并进注释 ⇒ 夹具**静默失效**（`DEBUG fns count=0` + `Get-FrontMatter not found`，即"守门人一直是死的"）。**为何门禁没拦住**：`syntax` 报 `.ps1:12文件/0失败` —— PS5.1 对无 BOM 文件按 ANSI 解码**不报错**（只是内容变乱码）⇒ **"语法通过" ≠ "语义正确"**（与 G8「'包在' ≠ '功能在'」同族）。**处置**：已补 BOM（改前备份 4802B 一致）恢复 `fns=32`。**⚠ 09-21 现场复现 + 根因精化（同族第二次咬人）**：编辑工具（Trae `Edit`/`SearchReplace`）**重写文件时不保留 BOM** ⇒ 09-21 对 `agent-cli.ps1` / `_fm_golden_test.ps1` / `_probe_fallback.ps1` 编辑后**三个文件 BOM 全丢**（实测：`git cat-file blob HEAD:<f>` 首 3 字节 `efbbbf`，工作区 `23 20 3D`），而 `rpc.ps1 check` 仍报 **`.ps1:13文件/0失败` + 整仓 PASS**；随后夹具**当场解析崩溃**（`Unexpected token 'claude'` + 中文乱码 `鍩虹嚎`）。**根因精化**：门禁用 [rpc_check.py](../../ops/rpc_check.py) `PS_SNIPPET` 的 **`[Parser]::ParseFile($f, …)`** —— 该 API 走 .NET 的解码（BOM-less 时按 **UTF-8**），而**实际执行** `powershell -File x.ps1` 走 PowerShell **自己的解码器（BOM-less ⇒ ANSI）** ⇒ **两者解码路径不同**，门禁**结构性地**看不到这个故障（不是"判据太弱"，是"判据看不到"）。**处置**：`tmp/_restore_bom.ps1` 补回 BOM + `Parser::ParseFile` 复检三件 `parseErrors=0`；**纪律**：凡编辑过 `.ps1`，提交前必须**按字节**验 BOM（不能信 `syntax`） | 待办：给 `syntax` 加**子判据** —— 含非 ASCII 的 `.ps1` 必须带 UTF-8 BOM（可机判、误报低）；**实现要点（09-21 精化）**：必须**读字节**判（`read_bytes()` 看 `efbbbf` + 是否存在非 ASCII 字节），**不能**靠 `Parser.ParseFile`（上面已证它不重现）；属新增能力，走 ADR-0004 D3。**⚠ 原待办范围需修正**：`.sh` **不得**加 BOM（BOM 会让内核把 `#!` 认成 `\xEF\xBB\xBF#!` ⇒ `bad interpreter` 直接不可执行）⇒ 子判据**只覆盖 `.ps1`**，`.sh` 的同类风险应另判（Linux 侧 locale 为 UTF-8，风险形态不同） |
