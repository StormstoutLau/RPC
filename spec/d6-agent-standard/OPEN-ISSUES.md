...
| ~~O-52~~ | 证据/采集面 | P2 | **A-② 只支持"单文件扁平名"** ⇒ 产物名不可知时取不回 | **✅ 机制已实施 · ★ 端到端复跑发现【更底层的新缺口】⇒ 转 O-53**（glob 分支本身工作正常：`EVM_STATE_GLOB` 前的字符集/固定枚举/恰 1 校验链均已跑通并被拒绝路径实证） | O-45 · O-40 · **O-53** |
| **O-53** | 证据/采集面 | **P1** | ★ **产物在 `accept` 通过之后、采集之前从 `$W/out/` 消失**（端到端三次取证交叉定位，见详情）⇒ **任何"产物即结论"的卡都可能收不回件**（含 A1/B1/B2 的产物为何只留站上、需人工 `scp` —— **此前归因"设计如此"，现在看可能是本条**） | ⏳ **待定位根因**（候选：`sync` 对 `out/` 的处理 / 框架在采集后按 subject 清理 / `agent-out` 暂存 Move 的副作用）· **不得先改代码**（先做数据侦察） | O-52 · O-40 · ADR-0007 |
