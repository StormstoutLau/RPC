...
| O-34 | 门禁/一致性 | P2 | **迁移/重命名后「首次」门禁为 FAIL(1 项失败 / 绿灯 9)，而提交时同一命令为 PASS(绿灯 10)** —— **已定位**：`_iter_source_files()` 以 `git ls-files` 取范围，其假设"**pre-commit 阶段新文件已 `git add`**"**只在钩子里成立**；手动直跑 + 未暂存 ⇒ 范围不同 | ◐ **已定位（有代码依据）**，修法待落地（落点需读 `secrets` 报数行） | D6-P1 |
...
#### O-34：迁移后**首次**门禁 FAIL，提交时同一命令 PASS —— **疑似断言对 git 暂存状态敏感**（**未定位**）
...
- ⇒ **修法（未落地）**：让 `_iter_source_files` 把"**范围已收窄**"显式暴露 ——
  用 `git ls-files --others --exclude-standard` 统计**未跟踪（⇒ 不计入门禁范围）**的文件数，
  **在报数里注明**（干净仓库时为 0 ⇒ **零噪声**）。
  ⚠ 落点需先读 `secrets` 断言的**报数行**（**本轮未读，故不改** —— 不盲改）。
