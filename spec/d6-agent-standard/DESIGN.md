...
主控站多个任务项目（RPC/Paper/Spec_Workflow/Auto_Prover/Cpp_Hub）需要调用两站 4 个 agent CLI，当前无统一调用规范、无项目间隔离、无并发保护、无敏感内容路由。本设计交付 **`agent-cli` wrapper MVP**（PowerShell）：单命令完成"工作区同步 → ssh headless 执行 → 产物回收"全链，内建并发锁（flock 双层）、任务卡状态机（孤儿锁可检测）、敏感内容三档硬路由（local-only 不可覆写）。为后续站间互审（D7+）与 trae 五层循环对接（D7）预留任务卡接口。
...
| 额度观测 | 无（打到限额才报错） | .agent-run.json 记 queue_s/run_s/tokens 三元组，本地 log 累计 |
...
| "Bash 不锁"系未验证假设（exec_command=true 归因是推断；opencode 内部序列化未测） | 列入 V0 验证门第 5 项，不通过则 Bash 调用也入锁 | 审计 R6/§9.7.1 |
...
| task-notification usage 三元组（tokens/tool_uses/duration）+ queue_s/run_s 分离计量 | .agent-run.json 契约吸收双方字段（G13 观测） | §9.6-2/§9.7.2 |
...
| 试点建议 Paper 起步（177 pytest 验收判据现成） | V0 试点项目 = Paper（第二试点 Cpp_Hub 列 T 后期） | §9.3 |
...
- 描述：B 站 claude 开 CLAUDE_CODE_COORDINATOR_MODE=1，LLM 在编排席
...
7. **门禁缓存不变式**：重试不重触发 audit/审批（Codex already_approved 语义）
...
| V0-4 | A 站记忆功能路径（G5 遗留） | A 站 memory_add_note 写入成功 | A 站任务全路由 B 站 |
...
| llama-server 单槽排队致超时 | timeout 默认 900s 宽裕 + queue_s 观测定位 |
