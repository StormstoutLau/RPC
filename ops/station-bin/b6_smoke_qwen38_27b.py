#!/usr/bin/env python3
# b6_smoke_qwen38_27b.py — B6 冒烟套件 on Qwen3.8-27B (Jackrong Q8_0 @ :18080)
# 题面逐字复用留档 b6_smoke_run.py + b6_c2_nothink.py; C2 按 DESIGN 决策6 禁思考
import json, time, urllib.request

API = "http://127.0.0.1:18080/v1/chat/completions"
OUT = "/tmp/b6_smoke_qwen38_27b.jsonl"
MODEL = "qwen3.8-27B-MTP"

QUESTIONS = [
    {
        "id": "dmx-a3", "type": "numeric", "max_tokens": 2048, "thinking": True,
    ...
    {
        "id": "dmx-b2", "type": "numeric", "max_tokens": 2048, "thinking": True,
    ...
    {
        "id": "dmx-c2", "type": "code", "max_tokens": 6144, "thinking": False,
    ...
    {
        "id": "dmx-a1", "type": "rubric", "max_tokens": 3072, "thinking": True,
        "prompt": ("高效作答, 思考尽量精简。概念题(分步作答, <=10 行): 设夹层保费 Phi_k(rho) 连续依赖相关参数 rho。\n"
                   "绝对灵敏度 = |Phi'(rho)|; 相对灵敏度 S = |d ln Phi / d rho| = |Phi'/Phi|。\n"
                   "第一步(本题只答这步): 构造一个具体例子说明 Phi'(rho*) 可以有限而 S(rho*) 任意大,\n"
                   "并指出这依赖什么机制(提示: 支付概率/期望趋小的夹层)。"),
    },
    {
        "id": "dmx-g1", "type": "rubric", "max_tokens": 3072, "thinking": True,
        "prompt": ("高效作答, 思考尽量精简。概念题(<=10 行): Epstein-Zin 递归效用将跨期替代弹性(IES, psi)与风险厌恶(gamma)分离,\n"
    ...
    for q in QUESTIONS:
        try:
            ...
            print(f"[{q['id']}] ok {r['elapsed_s']}s finish={r['finish']} "
                  ...
                  f"comp_tokens={r['usage'].get('completion_tokens')}")
